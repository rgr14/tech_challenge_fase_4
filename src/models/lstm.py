"""
Modulo de definicao do modelo LSTM - Replica do Original
"""
import numpy as np
import pandas as pd
...
class StockLSTMModel:
    ...
    def build_model(self) -> Sequential:
        """
        Replicando a funcao build_model do arquivo original:
        - 2 Camadas LSTM
        - Dropout apos cada uma
        ...
        model.add(LSTM(
            ...
            return_sequences=True,  # Necessario pois tem a 2a camada
        ))
        model.add(Dropout(self.dropout_rate))

        # 2a Camada (Original tinha n_layers=2)
        model.add(LSTM(
        ...
        self.model = model
        logger.info(f"Modelo construido (Replica Original): MSE Loss, 2 Layers, {self.lstm_units_1} units")
        return model
    ...
    def get_callbacks(self, ticker: str) -> List:
        callbacks = [
            ...
            # O original nao tinha ReduceLR explicitamente no grid search,
            # mas e boa pratica manter.
            ModelCheckpoint(
