"""
Modulo de coleta de dados de acoes via Yahoo Finance
"""
...
from src.config import DATA_DIR, DEFAULT_TICKER, DEFAULT_START_DATE, DEFAULT_END_DATE
...
class StockDataCollector:
    """Classe para coleta e gerenciamento de dados de acoes."""
    ...
    def fetch_historical_data(
        ...
        """
        Busca dados historicos da acao.
        ...
        Returns:
            DataFrame com dados historicos
        """
        if end_date is None:
            end_date = datetime.now().strftime("%Y-%m-%d")

        logger.info(f"Buscando dados de {self.ticker} de {start_date} ate {end_date}")
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Remover colunas desnecessarias se existirem
        cols_to_drop = ["Dividends", "Stock Splits"]
        df = df.drop(columns=[c for c in cols_to_drop if c in df.columns])

        # Garantir que o indice e datetime
    ...
    def get_latest_data(self, days: int = 60) -> pd.DataFrame:
        """
        Busca os ultimos N dias de dados.

        Args:
            days: Numero de dias para buscar
        ...
        # Retornar apenas os ultimos N dias uteis
    ...
    def get_stock_info(self) -> dict:
        """Retorna informacoes basicas da acao."""
